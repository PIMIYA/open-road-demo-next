...
export default function PDFViewer({ src }) {
  ...
  useEffect(() => {
    ...
    if (!pdf || !canvas || !totalPages) return;
  ...
  return (
    <Box ref={containerRef} sx={{ border: "1px solid", borderColor: "divider", display: "flex", flexDirection: "column", bgcolor: "background.paper", ...(isFullscreen && { height: "100vh" }) }}>
      {/* Controls */}
      <Box sx={{ borderBottom: "1px solid", borderColor: "divider", px: 1.5, py: 0.5, display: "flex", alignItems: "center", justifyContent: "space-between", bgcolor: "action.hover" }}>
        {/* Page nav */}
      ...
      {/* PDF Canvas */}
      <Box sx={{ height: isFullscreen ? 0 : 700, overflow: "auto", display: "flex", justifyContent: "center", alignItems: "flex-start", p: 2, bgcolor: "grey.100", flex: 1 }}>
        {error ? (
