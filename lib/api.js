...
export async function MainnetAPI(path) {
    try {
        ...
        console.error("MainnetAPI error:", error.message);
        return null;
    }
}
...
export async function TestnetAPI(path) {
    try {
        ...
        console.error("TestnetAPI error:", error.message);
        return null;
    }
}
...
export async function WalletRoleAPI(path) {
    try {
        ...
        const response = await fetch(requestUrl);
        const data = await response.json();
        return [1, 2, 3]; // TODO: return actual data
    } catch (error) {
        console.error("WalletRoleAPI error:", error.message);
        return null;
    }
}
...
export async function AkaDropAPI(path) {
    try {
        ...
        console.error("AkaDropAPI error:", error.message);
        return null;
    }
}
...
export async function TZKT_API(path) {
    try {
        ...
        console.error("TZKT_API error:", error.message);
        return null;
    }
}
...
/**
 * Batch version: fetch pool data ONCE and check multiple tokens.
 * Avoids N redundant API calls for the same contract/bigmap.
 */
export async function GetClaimablePoolIDBatch(contractAddress, targetContractAddress, tokenIds) {
    try {
        const requestUrl_storage = TZKT_URL(`/v1/contracts/${contractAddress}/storage`);
        const storageResponse = await axios.get(requestUrl_storage);
        const bigMapId = storageResponse.data['pools'];
        ...
        for (const tokenId of tokenIds) {
            const filtered = bigMapValues.filter(entry =>
                entry.value.tokens.some(token =>
                    token.key.fa2 === targetContractAddress && token.key.id === tokenId
                )
            );
            result[tokenId] = filtered.length > 0 ? filtered : null;
        }
        return result;
    } catch (error) {
        return {};
...
export async function FetchDirectusData(path) {
    try {
        ...
        console.error("FetchDirectusData error:", error.message);
        return null;
    }
}
