...
/**
 * Fetch event with its NFTs by event ID.
 * @param {string} eventId - Event identifier (e.g., 'gm-kairos')
 * @returns {Promise<Object>} Event object with nfts array
 */
export async function fetchEventNfts(eventId) {
  const res = await fetch(`${MAP_SERVER}/api/events/by-id/${eventId}`);
  if (!res.ok) {
    if (res.status === 404) return null;
    throw new Error(`Failed to load event NFTs for ${eventId}: ${res.status}`);
  }
  return res.json();
}
