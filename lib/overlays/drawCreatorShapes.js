...
export function getCreatorShapeMap(filteredNfts, artistNameMap) {
  ...
  const creators = [...addresses]
    .map((addr) => {
      const entry = nameMap.get(addr);
      const name = (typeof entry === "object" ? entry?.name : entry) || addr;
      const name_en = typeof entry === "object" ? (entry?.name_en || null) : null;
      return { addr, name, name_en };
    })
    .sort((a, b) => a.name.localeCompare(b.name));

  const result = new Map();
  creators.forEach(({ addr, name, name_en }, i) => {
