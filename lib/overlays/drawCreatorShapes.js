...
export function drawCreatorShapesLayer(ctx, filteredNfts, venues, creatorShapeMap, opts) {
  ...
  // Global grid prevents overlap across all NFTs.
  ...
  // Place shapes at each NFT's own position (not centroid),
  // so creators with NFTs at multiple venues don't land in between.
  for (const nft of filteredNfts) {
    const anchorX = nft.svgX;
    const anchorY = H - nft.svgY;

    for (const addr of nft.creators || []) {
      const info = creatorShapeMap.get(addr);
      if (!info) continue;

      const rand = seededRandom(hashSeed(nft.id + addr));

      // Spiral outward from this NFT to find a free cell
      let placed = false;
